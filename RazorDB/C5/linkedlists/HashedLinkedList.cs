...
, IStack<T>, IQueue<T>
#endif
  {
    ...
    class Range : DirectedCollectionValueBase<T>, IDirectedCollectionValue<T>
    {
      ...
      internal Range(HashedLinkedList<T> l, int s, int c, bool b)
      {
    ...
    void raiseForInsertAll(Node node, int i, int added, bool insertion)
    {
      if (ActiveEvents != 0)
      {
        ...
        if ((ActiveEvents & (EventTypeEnum.Added | EventTypeEnum.Inserted)) != 0)
          ...
          {
              // TODO: must we check stamps here?
            node = node.next;
            T item = node.item;
...
#pragma warning restore 472
				try {
					getPair(offset - 1, offset + size, out startsentinel, out endsentinel, new int[] { -1, size }, new Node[] { startsentinel, endsentinel }); //TODO: maybe-update offset field
